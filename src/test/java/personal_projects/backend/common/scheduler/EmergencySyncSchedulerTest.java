...
class EmergencySyncSchedulerTest {
    @Mock
    private EmergencyImporter emergencyImporter;

    @Test
    void 스케줄실행을_동기화서비스에_위임한다() {
        EmergencySyncScheduler scheduler = new EmergencySyncScheduler(emergencyImporter);

        scheduler.synchronize();

        verify(emergencyImporter).importAll();
    }
}
