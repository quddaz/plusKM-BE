...
public class PlaceJdbcRepository {
    ...
    public List<SearchResultPlaceResponse> findPlacesWithinBuffer(
        ...
        double latitudeDistance = bufferDistance / 111.32;
        double longitudeDistance = bufferDistance / (111.32 * Math.cos(Math.toRadians(latitude)));
        String polygon = createPolygon(longitude, latitude, longitudeDistance, latitudeDistance);

        StringBuilder sql = new StringBuilder("""
            SELECT id, name, address, tel, ST_X(coordinate) AS longitude, ST_Y(coordinate) AS latitude
            FROM place
            WHERE active = true
    ...
    public SearchDetailPlaceResponse findPlaceDetailByPlaceId(Long placeId, Long userId) {
        String sql = """
            SELECT p.id, p.name, p.address, p.tel, p.place_type,
                   EXISTS(
                       SELECT 1
                       FROM book_mark b
        ...
        return jdbcTemplate.query(sql, resultSet -> {
            ...
            return new SearchDetailPlaceResponse(
                resultSet.getLong("id"),
                resultSet.getString("name"),
                resultSet.getString("address"),
                resultSet.getString("tel"),
    ...
    public List<SearchBookMarkPlaceResponse> findBookMarkPlacesByUserId(Long userId) {
        ...
        return jdbcTemplate.query(sql, (resultSet, rowNumber) ->
            new SearchBookMarkPlaceResponse(
                resultSet.getLong("id"),
                resultSet.getString("name"),
                resultSet.getString("address"),
                resultSet.getString("tel"),
    ...
    public void batchUpsert(List<Place> places) {
        ...
        jdbcTemplate.batchUpdate(sql, places, BATCH_SIZE, (statement, place) -> {
            ...
            statement.setString(2, place.getPlace_type().name());
            statement.setString(3, place.getAddress());
            statement.setString(4, place.getTel());
